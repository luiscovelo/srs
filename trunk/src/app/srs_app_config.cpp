            ...
            if (n == "dvr") {
                for (int j = 0; j < (int)conf->directives.size(); j++) {
                    ...
                    if (m != "enabled"  && m != "dvr_apply" && m != "dvr_path" && m != "dvr_plan"
                        && m != "dvr_duration" && m != "dvr_wait_keyframe" && m != "time_jitter" && m != "dvr_app_name_to_ignore_audio") {
...
{   
    // SRS_VHOST_DVR_DVR_APP_NAME_TO_IGNORE_AUDIO
    SRS_OVERWRITE_BY_ENV_DIRECTIVE("srs.vhost.dvr.dvr_app_name_to_ignore_audio");
