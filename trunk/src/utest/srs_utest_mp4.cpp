...
VOID TEST(ReproduceIssue4625, PreserveZeroDtsInMp4Stts)
{
    ...
    for (vector<SrsMp4SttsEntry>::iterator it = stts.entries.begin(); it != stts.entries.end(); ++it) {
        duration += (uint64_t)it->sample_count * it->sample_delta;
    ...
    ASSERT_EQ(3, (int)stts.entries.size());
    EXPECT_EQ(1, (int)stts.entries[0].sample_count);
    EXPECT_EQ(9, (int)stts.entries[0].sample_delta);
    EXPECT_EQ(3, (int)stts.entries[1].sample_count);
    EXPECT_EQ(0, (int)stts.entries[1].sample_delta);
    EXPECT_EQ(1, (int)stts.entries[2].sample_count);
    EXPECT_EQ(30, (int)stts.entries[2].sample_delta);
}
